...
const { planSelect } = require("../../query");
const { buildSelectSql } = require("./sql-builder");
...
function adultFromRow(row) {
  return { ...rowExtra(row), id: row.id, name: row.name, relationship: row.relationship, email: row.email, homePhone: row.home_phone, cellPhone: row.cell_phone };
}
...
async function select(resource, query = {}, context = {}) {
  const plan = planSelect(resource, query, context);
  const { sql, values } = buildSelectSql(plan);
...
async function getAdultsByIds(ids) {
  const safeIds = Array.isArray(ids) ? ids.map((id) => String(id || "").trim()).filter(Boolean) : null;
  if (safeIds && !safeIds.length) {
    return [];
  }
  const result = safeIds
    ? await getPool().query("SELECT * FROM adults WHERE id = ANY($1) ORDER BY id", [safeIds])
    : await getPool().query("SELECT * FROM adults ORDER BY id");
  return result.rows.map(adultFromRow);
}
...
async function saveAdult(adult) {
  ...
  return adultFromRow(saved.rows[0]);
}

async function deleteAdult(adultId) {
  const result = await withTransaction(async (client) => {
    const deleted = await client.query("DELETE FROM adults WHERE id = $1", [adultId]);
    await client.query("DELETE FROM adult_leaders WHERE adult_id = $1", [adultId]);
    await client.query("DELETE FROM adult_scout_relationships WHERE adult_id = $1", [adultId]);
    return deleted;
  });
  return result.rowCount > 0;
}
...
async function getDataPayload() {
  ...
  return {
    ...
    adults: adults.rows.map(adultFromRow),
