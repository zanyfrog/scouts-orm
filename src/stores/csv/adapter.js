...
const db = require("../postgres/adapter");
const { matchesWhere, planSelect, projectRows } = require("../../query");
...
const adultFieldNames = ["id", "name", "relationship", "email", "homePhone", "cellPhone"];
...
function adultFromCsvDatabaseRow(row) {
  return {
    ...rowExtra(row),
    id: row.id,
    name: row.name,
...
function normalizeAdult(adult) {
  const source = adult || {};
  return {
    ...withoutFields(source, adultFieldNames),
    id: String(source.id || "").trim(),
    name: String(source.name || "").trim(),
    relationship: String(source.relationship || "").trim(),
    email: String(source.email || "").trim(),
    homePhone: String(source.homePhone || "").trim(),
...
function getCsvDatabasePayload() {
  ...
  return {
    ...
    adults: readCsv(csvDatabaseFiles.adults).map(adultFromCsvDatabaseRow),
...
function getCsvDatabaseEvents({ includeMedia = true } = {}) {
  const activityMap = new Map();
  readCsv(csvDatabaseFiles.eventActivities).forEach((row) => {
    const eventId = row.event_id;
    if (!activityMap.has(eventId)) {
      activityMap.set(eventId, []);
    }
  ...
  const mediaMap = new Map();
  if (includeMedia) {
    readCsv(csvDatabaseFiles.eventMedia).forEach((row) => {
      const eventId = row.event_id;
      if (!mediaMap.has(eventId)) {
        mediaMap.set(eventId, []);
      }
  ...
  return readCsv(csvDatabaseFiles.events).map((row) => csvEventFromRow(
    row,
    (activityMap.get(row.id) || []).sort((a, b) => a.position - b.position).map((item) => item.activity),
    mediaMap.get(row.id) || [],
    includeMedia
...
function applyOrder(rows, orderBy = []) {
  ...
  return [...rows].sort((a, b) => {
    for (const item of orderBy) {
      ...
      const result = compareValues(a[field], b[field], String(item.direction || "asc").toLowerCase());
...
function select(resource, query = {}, context = {}) {
  if (db.enabled()) {
    return db.select(resource, query, context);
  }
  const plan = planSelect(resource, query, context);
  const payload = getDataPayload();
  const rows = Array.isArray(payload[resource]) ? payload[resource] : [];
  const filtered = rows.filter((row) => matchesWhere(row, plan.where));
  const ordered = applyOrder(filtered, plan.orderBy);
  const offset = Math.max(0, Number(plan.offset) || 0);
  const limited = plan.limit === undefined || plan.limit === null
    ? ordered.slice(offset)
    : ordered.slice(offset, offset + Math.max(0, Number(plan.limit) || 0));
  return {
    rows: projectRows(limited, plan.columns),
    columns: plan.columns,
    omittedColumns: plan.omittedColumns,
...
function getAdultsByIds(ids) {
  const safeIds = Array.isArray(ids) ? ids.map((id) => String(id || "").trim()).filter(Boolean) : null;
  if (safeIds && !safeIds.length) {
    return db.enabled() ? Promise.resolve([]) : [];
  }
  if (db.enabled()) {
    return db.getAdultsByIds(safeIds);
  }
  const adults = csvDatabaseEnabled() ? readCsv(csvDatabaseFiles.adults).map(adultFromCsvDatabaseRow) : readCsv(files.adults);
  if (!safeIds) {
    return adults;
  }
  const idSet = new Set(safeIds);
  return adults.filter((adult) => idSet.has(String(adult.id)));
}
...
function getEvents({ startDate, endDate, page = 1, pageSize = 50, includeMedia = false } = {}) {
  ...
  const allEvents = csvDatabaseEnabled()
    ? getCsvDatabaseEvents({ includeMedia })
    : readJson(files.events, []).map(normalizeEvent);
...
function getEventById(eventId, { includeMedia = true } = {}) {
  ...
  const event = (csvDatabaseEnabled()
    ? getCsvDatabaseEvents({ includeMedia })
    : readJson(files.events, [])).find((item) => String(item.id) === String(eventId));
  if (!event) {
    return null;
  }
...
async function saveAdult(adult) {
  const normalizedAdult = normalizeAdult(adult);
  if (!normalizedAdult.id) {
    throw new Error("Adult id is required");
  }
  if (db.enabled()) {
    return db.saveAdult(normalizedAdult);
  }
  if (csvDatabaseEnabled()) {
    const adults = readCsv(csvDatabaseFiles.adults).map(adultFromCsvDatabaseRow);
    const index = adults.findIndex((item) => String(item.id) === String(normalizedAdult.id));
    if (index === -1) {
      adults.push(normalizedAdult);
    } else {
      adults[index] = normalizedAdult;
    }
    saveCsvDatabaseAdults(adults);
    return normalizedAdult;
  }

  const adults = readCsv(files.adults);
  const index = adults.findIndex((item) => String(item.id) === String(normalizedAdult.id));
  if (index === -1) {
    adults.push(normalizedAdult);
  } else {
  ...
  writeCsv(
    files.adults,
    adultHeaders,
    adults.map((item) => [item.id, item.name, item.relationship, item.email, item.homePhone || "", item.cellPhone || ""])
  );
  return normalizedAdult;
}

async function deleteAdult(adultId) {
  const safeAdultId = String(adultId || "").trim();
  if (!safeAdultId) {
    return false;
  }
  if (db.enabled()) {
    return db.deleteAdult(safeAdultId);
  }

  const payload = getFileDataPayload();
  const nextAdults = payload.adults.filter((adult) => String(adult.id) !== safeAdultId);
  if (nextAdults.length === payload.adults.length) {
    return false;
  }
  const nextAdultLeaders = payload.adultLeaders.filter((leader) => String(leader.adultId) !== safeAdultId);
  const nextAdultScoutRelationships = payload.adultScoutRelationships.filter((relationship) => String(relationship.adultId) !== safeAdultId);

  if (csvDatabaseEnabled()) {
    saveCsvDatabaseAdults(nextAdults);
    saveCsvDatabaseAdultLeaders(nextAdultLeaders);
    saveCsvDatabaseAdultScoutRelationships(nextAdultScoutRelationships);
  } else {
    saveAdults(nextAdults);
    saveAdultLeaders(nextAdultLeaders);
    saveAdultScoutRelationships(nextAdultScoutRelationships);
  }
  return true;
}
