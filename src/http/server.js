...
const { port, authBaseUrl, internalServiceToken, readCacheTtlMs } = require("../config/env");
const {
  roles,
  hasAnyRole,
  hasAdministrator,
...
async function resolvePublicActor(req) {
...
function bodyAdult(body, fallbackId = "") {
  const source = body.adult && typeof body.adult === "object" && !Array.isArray(body.adult) ? body.adult : body;
...
function stripPrivateEventFields(event) {
  const { homeBase, startDate, endDate, ...publicEvent } = event || {};
  return publicEvent;
}
...
async function handleApi(req, res) {
  ...
  const reservationMatch = url.pathname.match(/^\/api\/events\/([^/]+)\/reservations\/([^/]+)$/);
  if (reservationMatch && (req.method === "POST" || req.method === "DELETE")) {
    const actor = await requireActor(req, res, hasMemberAccess);
    if (!actor) return true;
    const data = await loadDataPayload();
    const eventId = decodeURIComponent(reservationMatch[1]);
    const personId = decodeURIComponent(reservationMatch[2]);
    const eventExists = data.events.some((event) => event.id === eventId);
    const isOperationalEditor = hasAdultLeaderScoutAccess(actor);
    if (!eventExists) {
      json(res, 404, { error: "Event not found" });
      return true;
    }
    if (!isOperationalEditor && !canRegisterForPerson(actor, personId, data)) {
      forbidden(res, actor);
      return true;
    }
    if (req.method === "DELETE") {
      const result = await orm.removeEventRegistration(eventId, personId);
      invalidateReadCaches();
      json(res, 200, result);
      return true;
    }
    const registration = await orm.registerForEvent(eventId, personId, { registeredByPersonId: actorPersonId(actor) });
    invalidateReadCaches();
  ...
  if (req.method === "GET" && url.pathname === "/api/adults") {
    const actor = await requireActor(req, res, hasOperationalAccess);
    if (!actor) return true;
    const ids = url.searchParams.has("ids") ? parseScoutIdsParam(url.searchParams.get("ids")) : null;
    json(res, 200, { adults: await orm.getAdultsByIds(ids) });
    return true;
  }

  if (req.method === "GET" && url.pathname.startsWith("/api/adults/")) {
    const actor = await requireActor(req, res, hasOperationalAccess);
    if (!actor) return true;
    const adultId = decodeURIComponent(url.pathname.slice("/api/adults/".length));
    const adult = (await orm.getAdultsByIds([adultId]))[0];
    if (!adult) {
      json(res, 404, { error: "Adult not found" });
      return true;
    }
    json(res, 200, { adult });
    return true;
  ...
  if (req.method === "POST" && url.pathname === "/api/adults") {
    ...
    if (!actor) return true;
    const body = JSON.parse((await readBody(req)) || "{}");
    if (Array.isArray(body.adults)) {
      await orm.saveAdults(body.adults);
      invalidateReadCaches();
      json(res, 200, { ok: true });
      return true;
    }
    const adult = bodyAdult(body);
    if (!adult) {
      json(res, 400, { error: "An adult object with an id or an adults array is required" });
      return true;
    }
    const savedAdult = await orm.saveAdult(adult);
    invalidateReadCaches();
    json(res, 200, { adult: savedAdult });
    return true;
  }

  if ((req.method === "POST" || req.method === "PUT" || req.method === "PATCH") && url.pathname.startsWith("/api/adults/")) {
    const actor = await requireActor(req, res, hasOperationalWriteAccess);
    if (!actor) return true;
    const adultId = decodeURIComponent(url.pathname.slice("/api/adults/".length));
    const body = JSON.parse((await readBody(req)) || "{}");
    const adult = bodyAdult(body, adultId);
    if (!adult) {
      json(res, 400, { error: "An adult object is required" });
      return true;
    }
    const existingAdult = req.method === "PATCH" ? (await orm.getAdultsByIds([adultId]))[0] : null;
    const savedAdult = await orm.saveAdult({ ...(existingAdult || {}), ...adult, id: adultId });
    invalidateReadCaches();
    json(res, 200, { adult: savedAdult });
    return true;
  }

  if (req.method === "DELETE" && url.pathname.startsWith("/api/adults/")) {
    const actor = await requireActor(req, res, hasOperationalWriteAccess);
    if (!actor) return true;
    const adultId = decodeURIComponent(url.pathname.slice("/api/adults/".length));
    const deleted = await orm.deleteAdult(adultId);
    if (!deleted) {
      json(res, 404, { error: "Adult not found" });
      return true;
    }
